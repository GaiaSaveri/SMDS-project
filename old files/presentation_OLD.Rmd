...
```{r exp_dataset}
colombia_covid2 <- as.data.frame(read_csv("data/covid19co.csv"))
cols <- colnames(colombia_covid2)[c(1, 4, 5, 6, 7, 8, 9, 11, 14)]
colombia_covid2 <- colombia_covid2[cols]
colombia_covid2 <- colombia_covid2[, c(1, 9, 2, 3, 4, 5, 6, 7, 8)]
colnames(colombia_covid2) <- c("ID de caso", "Fecha de diagnóstico", "Ciudad de ubicación", "Departamento o Distrito", "Atención", "Edad" , "Sexo", "Tipo", "País de procedencia")
colombia_covid2$`Departamento o Distrito`[which(colombia_covid2$`Departamento o Distrito` == "Valle Del Cauca")] <- "Valle del Cauca"
central.colombia.rows2 <- which(colombia_covid2$`Departamento o Distrito` %in% central.colombia.dep)
colombia_covid2 <- colombia_covid2[central.colombia.rows2, ]
colombia_covid2 <- colombia_covid2[-which(colombia_covid2$`Fecha de diagnóstico`== "-"), ]
colombia_covid2$`Fecha de diagnóstico` <- as.Date(colombia_covid2$`Fecha de diagnóstico`, format="%d/%m/%Y")
colombia_covid2 <- colombia_covid2[order(colombia_covid2$`Fecha de diagnóstico`), ]
colombia_covid2$`ID de caso` <- 1:dim(colombia_covid2)[1]
colombia_covid2 <- colombia_covid2 %>% mutate("Grupo de edad" = case_when(Edad <= 18 ~ '0_18',
...
#View(colombia_covid[, 1:10] == colombia_covid2[1:993, ])
```
...
```{r cases2}
cases_2 <- colombia_covid2 %>%
  group_by(`Fecha de diagnóstico`) %>%
  count() %>% rename("Date" = `Fecha de diagnóstico`, "New cases/day" = n)
cases2_2 <- colombia_covid2 %>%
  group_by(`Fecha de diagnóstico`) %>%
  summarise(`Cumulative cases` = max(`ID de caso`))
cases_2 <- bind_cols(cases_2, cases2_2%>%
  dplyr::select(-c(`Fecha de diagnóstico`)))
cases_2 <- as.data.frame(cases_2)
cases_2 <- cases_2 %>% mutate(BETWEEN0 = as.numeric(difftime(Date, lag(Date, 1), units = "days")),
            BETWEEN = ifelse(is.na(BETWEEN0), 0, BETWEEN0), `Elapsed time` =
              cumsum(as.numeric(BETWEEN))) %>% dplyr::select(-c(BETWEEN0,BETWEEN))
cases_2 <- cases_2[, c(1,4,2,3)]
```
...
```{r dataset_Giullia2, include=FALSE}
covid19_2 <- dplyr::select(colombia_covid2, -c(`Ciudad de ubicación`,`Atención`,`Tipo`))
library(fastDummies)
covid19_dummy2 <- dummy_cols(covid19_2, select_columns = c("Departamento o Distrito", "Grupo de edad", "Sexo", "País de procedencia"), remove_first_dummy = TRUE, ignore_na=TRUE, split="-", remove_selected_columns=TRUE)
group_dummy2 <- covid19_dummy2 %>%
  group_by(`Fecha de diagnóstico`) %>%
  summarise_all(funs(sum)) %>%
  dplyr::select(-c(`Fecha de diagnóstico`,`ID de caso`))
data2 <- bind_cols(cases_2, group_dummy2)
```

## Poisson with `Elapsed time` as predictor

```{r pois_time, echo=TRUE, warning=FALSE}
...
#k=2
#n-k=20
print(paste("Estimated overdispersion", sum(res.st^2)/20))
poisson1.pred <- predict(poisson1, newdata = cases[23:25, ], type="response")
paste("RMSE:", sqrt(mean((poisson1.pred - cases$`Cumulative cases`[23:25])^2)))
...
```{r pois_time/dep, echo=TRUE, warning=FALSE}
...
```{r things, echo=TRUE, warning=FALSE}
...
```{r pois_time2, echo=TRUE, warning=FALSE}
...
```{r pois2, echo=TRUE, warning=FALSE}
...
paste("RMSE:", sqrt(mean((poisson2.pred - data1$`Cumulative cases`[23:25])^2)))
...
```{r pois3, echo=TRUE, warning=FALSE}
...
paste("RMSE:", sqrt(mean((poisson3.pred - data1$`Cumulative cases`[23:25])^2)))
...
```{r pois4, echo=TRUE, warning=FALSE}
...
paste("RMSE:", sqrt(mean((poisson4.pred - data1$`Cumulative cases`[23:25])^2)))
...
```{r pois4_2, echo=TRUE, warning=FALSE}
poisson4_2 <- glm(`Cumulative cases` ~ `Elapsed time` + `Grupo de edad_19_30` + `Grupo de edad_31_45` + `Grupo de edad_46_60` + `Grupo de edad_60_75` + `Grupo de edad_76+` + `Departamento o Distrito_Bogotá D.C.` + `Departamento o Distrito_Boyacá` + `Departamento o Distrito_Caldas` + `Departamento o Distrito_Casanare` + `Departamento o Distrito_Cauca` + `Departamento o Distrito_Cundinamarca` + `Departamento o Distrito_Meta` + `Departamento o Distrito_Quindío` + `Departamento o Distrito_Risaralda` + `Departamento o Distrito_Santander` + `Departamento o Distrito_Tolima`, data=data2[1:120, ], family=poisson)
# MISSING `Departamento o Distrito_Valle del Cauca` !!!!!!!!!!!!!!!!!!!!!!!!
par(mfrow=c(2,2))
plot(poisson4_2)
pred.pois4_2 <- poisson4_2$fitted.values
res.st4_2 <- (data2$`Cumulative cases` - pred.pois4_2)/sqrt(pred.pois4_2)
#n=25 
#k=19
#n-k=6
print(paste("Estimated overdispersion", est.overdispersion <- sum(res.st4_2^2)/6))
poisson4.pred_2 <- predict(poisson4_2, newdata = data2[120:126, ], type="response")
paste("Real: ", data2$`Cumulative cases`[120:126], "Predict: ", poisson4.pred_2)
paste("RMSE:", sqrt(mean((poisson4.pred_2 - data2$`Cumulative cases`[120:126])^2)))
#paste("MSE:", mean(poisson4$residuals^2))
paste("AIC:", poisson4_2$aic)
paste(c("Null deviance: ", "Residual deviance:"),
       round(c(poisson4_2$null.deviance, deviance(poisson4_2)), 2))
```

## Poisson with `Elapsed time`, `Age` and `Departments` as predictors for `New cases/day`

```{r pois4day, echo=TRUE, warning=FALSE}
...
#k=19
#n-k=6
print(paste("Estimated overdispersion", est.overdispersion <- sum(res.st4bis^2)/6))
poisson4bis.pred <- predict(poisson4bis, newdata = data1[23:25, ], type="response")
paste("RMSE:", sqrt(mean((poisson4bis.pred - data1$`New cases/day`[23:25])^2)))
#paste("MSE:", mean(poisson4$residuals^2))
...
```{r pois5, echo=TRUE, warning=FALSE}
...
poisson5.pred <- predict(poisson5, newdata = data1[23:25, ], type="response")
paste("RMSE:", sqrt(mean((poisson5.pred - data1$`Cumulative cases`[23:25])^2)))
...
par(mfrow=c(2,2))
plot(poisson6)
poisson6.pred <- predict(poisson6, newdata = data1[23:25, ], type="response")
paste("RMSE:", sqrt(mean((poisson6.pred - data1$`Cumulative cases`[23:25])^2)))
...
attach(data1)
conf.df_pois <- predict.confidence(poisson4, newdata = data1[23:25])
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois$lwr[1:22], ymax = conf.df_pois$upr[1:22]),
              data = data1[1:22, ],
              fill = color_scheme_get("blue")[[2]]) + 
  #geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois$lwr[23:25], ymax = conf.df_pois$upr[23:25]),
  #            data = data1[22:25, ],
  #            fill = color_scheme_get("red")[[2]]) +
  geom_line(aes(x = `Elapsed time`, y = conf.df_pois$fit[1:22]),
              data = data1[1:22, ],
...
```{r pre_acc_pois_2, warning=FALSE, include=FALSE}
attach(data2)
conf.df_pois2 <- predict.confidence(poisson4_2, newdata = data2[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
print(conf.df_pois2)
n <- 25
freq_coverage_pois2 <- sum(`Cumulative cases` >= conf.df_pois[, 2] & `Cumulative cases` <= conf.df_pois[, 3])
freq_coverage_pois2 <- freq_coverage_pois/n
detach(data2)
```

```{r precc_acc_pois_plot_2, echo=FALSE, warning=FALSE}
ggplot(data2, aes(`Elapsed time`, `Cumulative cases`)) +
  ...
  geom_line(aes(x = `Elapsed time`, y = conf.df_pois2$fit[1:120]),
              ...
              color = color_scheme_get("blue")[[4]], 
              size = 1.1) +
  geom_line(aes(x= `Elapsed time`, y = poisson4.pred_2),
              data = data2[120:126, ],
              color = color_scheme_get("red")[[4]],
              size = 1.1) +
  geom_point(aes(x = `Elapsed time`, y = `Cumulative cases`)) +
...
attach(data1)
conf.df_pois2 <- predict.confidence(poisson4bis, newdata = as.data.frame(cbind(data1$`Elapsed time`, data1$`New cases/day`)))
...
```{r precc_acc_pois_plot2, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `New cases/day`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois2$lwr[1:22], ymax = conf.df_pois2$upr[1:22]),
              data = data1[1:22, ],
              fill = color_scheme_get("blue")[[2]]) + 
  #geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois$lwr[22:25], ymax = conf.df_pois$upr[22:25]),
  #            data = data1[22:25, ],
  #            fill = color_scheme_get("red")[[2]]) +
  geom_line(aes(x = `Elapsed time`, y = conf.df_pois2$fit[1:22]),
              data = data1[1:22, ],
              color = color_scheme_get("blue")[[4]], 
              size = 1.1) +
...
nb1 <- glm.nb(`Cumulative cases` ~ `Elapsed time`, data=data1[1:22, ])
par(mfrow=c(2,2))
plot(nb1)
#n=25, k=2, n-k=23
...
paste("RMSE:", sqrt(mean((nb1.pred - data1$`Cumulative cases`[23:25])^2)))
...
par(mfrow=c(2,2))
plot(nb2)
nb2.pred <- predict(nb2, newdata = data1[23:25, ], type = "response")
paste("RMSE:", sqrt(mean((nb2.pred - data1$`Cumulative cases`[23:25])^2)))
...
par(mfrow=c(2,2))
plot(nb3)
nb3.pred <- predict(nb3, newdata = data1[23:25, ], type = "response")
paste("RMSE:", sqrt(mean((nb3.pred - data1$`Cumulative cases`[23:25])^2)))
...
paste("RMSE:", sqrt(mean((nb5.pred - data1$`Cumulative cases`[23:25])^2)))
...
attach(data1)
conf.df_nb <- predict.confidence(nb5, newdata = as.data.frame(cbind(data1$`Elapsed time`, data1$`Cumulative cases`)))
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_nb$lwr[1:22], ymax = conf.df_nb$upr[1:22]),
              data = data1[1:22, ],
              fill = color_scheme_get("blue")[[2]]) + 
  #geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_nb$lwr[22:25], ymax = conf.df_nb$upr[22:25]),
  #            data = data1[22:25, ],
  #            fill = color_scheme_get("red")[[2]]) +
  geom_line(aes(x = `Elapsed time`, y = conf.df_nb$fit[1:22]),
              data = data1[1:22, ],
              color = color_scheme_get("blue")[[4]], 
              size = 1.1) +
...
#compile the model 
model3.NB <- nstan_model("stan/NB_model3.stan")
...
<img src="stan_figs/looic.png" alt="looic" style="zoom:60%;" class="center"/>