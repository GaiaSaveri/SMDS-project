...
Let's have a look now at the country of origin variable.

```{r country, echo=TRUE, warning=FALSE}
country <- colombia_covid %>%
  group_by(`País de procedencia`)%>%
  count()
country
```

There are 56 categories in this variable. Is too fragmented to use it in an analysis. Therefore, we decided to assign the values to continents to be able to include this variable in our analysis. We divded it into the folowing categories: Africa, Asia, Europe, North America, Central America, South America and Colombia. We divided America into 3 parts because many cases are coming from America and America is big. Colombia we kept separated because we have to distinguish internal cases from external. 

### Steps to clean data and assign values to continents
...
```{r country1, echo=TRUE, warning=FALSE}
#There is an observation that has a 0 as País de procedencia
which(colombia_covid$`País de procedencia`=="0")
colombia_covid[911,] 
...
```{r country2, echo=TRUE, warning=FALSE}
...
>>>>>>> 674a7f8c5a0368b6e19de9573d884acb167df8d8
