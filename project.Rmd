...
cumulative2
<<<<<<< HEAD
#write.csv(cumulative2, "data/SIR_data.csv")
```

##Giullia's part

```{r Giullia1, echo=TRUE, warning=FALSE}
#Manipulation of the dataset to use it to run the models

#Dropping the variables "Ciudad de ubicación","Atención**", "Tipo*" and "`País de procedencia`".
#Considering the variable "Ciudad de ubicación" I decided to keep just the departments variable to focus the analysis in diving the region in departments and not in cities.
...
#Considering the variable "Tipo*" many observations are "in study", so it is better to use just the variable country of origin (`País de procedencia`), because they consider the ones "in study" as Colombia. However, the variable `País de procedencia` is very dirty and we have to prepare it to be able to use it. So as a first analysis I won't consider it. 
covid19 <- select(colombia_covid,-c(`Ciudad de ubicación`,`Atención**`,`Tipo*`,`País de procedencia`))
...
#Considering the variable "Atención**" at the moment I think is not relevant to know how many confirmed cases there will be, but we can add later and check. 
#Considering the variable "Tipo*" many observations are "in study", so it is better to use just the variable country of origin (`País de procedencia`), because they consider the ones "in study" as Colombia. However, the variable `País de procedencia` is very dirty and we have to prepare it to be able to use it. So to simplify as a first analysis I won't consider it.
...
library(fastDummies)
covid19_dummy <- dummy_cols(covid19, select_columns = c("Departamento o Distrito","Edad","Sexo"),remove_first_dummy = TRUE,ignore_na=TRUE,split="-",remove_selected_columns=TRUE)

#fix day column in "international" format so that R can fix the sorting properly
covid19_dummy$`Fecha de diagnóstico`<-as.Date(covid19_dummy$`Fecha de diagnóstico`, format="%d/%m/%Y")

#Grouping the dummy variables by date and deleting the `Fecha de diagnóstico` and `ID de caso` because I will bind with the cumulative table made by Gaia. 
=======
```
...
#Counting the occurance of the dummy variables by date and deleting the `Fecha de diagnóstico` and `ID de caso` because I will bind with the cumulative table made by Gaia.
>>>>>>> d1161e2cf231d60c7b1be97bc1eca5dbee8877f1
...
data1 <- bind_cols(cumulative,group_dummy)
data1
```
...
```{r Giullia1e, echo=TRUE, warning=FALSE}
#Biding
...
### `Cumulative confirmed` as response and `Fecha de diagnóstico` as predictor.
>>>>>>> d1161e2cf231d60c7b1be97bc1eca5dbee8877f1

```{r Giullia2, echo=TRUE, warning=FALSE}
#Running poisson with just the variable representing the time 
attach(data1)
poisson1 <- glm(`Cumulative confirmed`~`Fecha de diagnóstico`,family=poisson)
summary(poisson1)
```
...
res.st <- (`Cumulative confirmed`-pred.pois)/sqrt(pred.pois)

<<<<<<< HEAD
#plotting it again to check using the default R command
...
>>>>>>> d1161e2cf231d60c7b1be97bc1eca5dbee8877f1
par(mfrow=c(1,2))
plot(poisson1, which=c(1,3))
```

So the residuals are also distributed according to a Poisson. This is surprising and I don't know what does it mean. I have to research.
...
poisson1quasi <- glm(`Cumulative confirmed`~`Fecha de diagnóstico`,family=quasipoisson)
summary(poisson1quasi)

#plotting standardized residuals versus the fitted values
pred.poisq <- poisson1quasi$fitted.values
<<<<<<< HEAD
res.stq <- (`Cumulative confirmed`-pred.poisq)/sqrt(pred.poisq)

#calculating overdispersion
est.overdispersion <- sum(res.stq^2)/16
est.overdispersion
```

So I applied the quasi poisson and nothing changed. Have to understand what is going on here.
=======
res.stq <- (`Cumulative confirmed`-pred.poisq)/sqrt(summary(poisson1quasi)$dispersion*pred.poisq)
...
So I applied the quasi poisson and the overdispersion reduced to 1, which is very good. Anyway, the residuals are still behaving like a distribution. Have to understand why is that.


### `Cumulative confirmed` as response and `Fecha de diagnóstico` + `Sexo_M` as predictors.
>>>>>>> d1161e2cf231d60c7b1be97bc1eca5dbee8877f1

```{r Giullia6, echo=TRUE, warning=FALSE}
#Running poisson with the variable representing time plus gender
poisson2 <- glm(`Cumulative confirmed`~`Fecha de diagnóstico`+ Sexo_M,family=poisson)
summary(poisson2)
```

Results show that the variable gender is not signficant and thus doesn't change the performance of the model

<<<<<<< HEAD
=======

### `Cumulative confirmed` as response and `Fecha de diagnóstico` + the gender variables `Edad_19 a 30`, `Edad_31 a 45`, `Edad_46 a 60`, `Edad_60 a 75` and `Edad_76+` as predictors.

>>>>>>> d1161e2cf231d60c7b1be97bc1eca5dbee8877f1
```{r Giullia7, echo=TRUE, warning=FALSE}
#Running poisson with the variable representing time plus the age variables
poisson3 <- glm(`Cumulative confirmed`~`Fecha de diagnóstico`+`Edad_19 a 30`+`Edad_31 a 45`+`Edad_46 a 60`+`Edad_60 a 75`+`Edad_76+`,family=poisson)
summary(poisson3)
```
...
res.st3 <- (`Cumulative confirmed`-pred.pois3)/sqrt(pred.pois3)
<<<<<<< HEAD

est.overdispersion <- sum(res.st3^2)/16
est.overdispersion
```

The plot shows the same behavior of the model poisson1 but the overdispersion parameter decreased
...
```{r Giullia9, echo=TRUE, warning=FALSE}
#Let's apply a quasi poisson and see what happens
poisson3quasi <- glm(`Cumulative confirmed`~`Fecha de diagnóstico`+`Edad_19 a 30`+`Edad_31 a 45`+`Edad_46 a 60`+`Edad_60 a 75`+`Edad_76+`,family=quasipoisson)
summary(poisson3quasi)

#plotting standardized residuals versus the fitted values
pred.poisq3 <- poisson3quasi$fitted.values
res.stq3 <- (`Cumulative confirmed`-pred.poisq3)/sqrt(summary(poisson3quasi)$dispersion*pred.poisq3)
...
So I applied the quasi poisson and the overdispersion reduced to 1, which is very good and again the residuals are still behaving like a distribution.


### `Cumulative confirmed` as response and all the other variables as predictors.

```{r Giullia10, echo=TRUE, warning=FALSE}
>>>>>>> d1161e2cf231d60c7b1be97bc1eca5dbee8877f1
#Finally, running poisson with the variable representing time plus the age variables and the departments variables (also runned gender together because it is easy to encode, but later can remove)
poisson4 <- glm(`Cumulative confirmed`~.,data=data1,family=poisson)
summary(poisson4)
```

The result of the significance of the variables is awkward, because just `Fecha de diagnóstico` has high significance but anyway the AIC reduced by half in comparisson to the poisson3 model!

<<<<<<< HEAD
```{r Giullia10, echo=TRUE, warning=FALSE}
#plotting the residuals versus fit and calculating the overdispersion 
pred.pois4 <- poisson4$fitted.values
res.st4 <- (`Cumulative confirmed`-pred.pois4)/sqrt(pred.pois4)

est.overdispersion <- sum(res.st4^2)/16
est.overdispersion
```

The plot shows the same behavior of the model poisson1 but the overdispersion parameter decreased significantly!!!
=======
...
#Calculating the overdispersion
#calculating overdispersion
#n=25 
#k=20
#n-k=5
est.overdispersion <- sum(res.st4^2)/5
est.overdispersion
```

Now the residuals are behaving differently and the overdispersion reduced significantly and it is inside the range assumed by a Poisson model, that is from -1 to 1.
...
colombia_covid[911,] # this one has 0 as País de procedencia
...
# What to do with "Jamaica - Panamá - Islas del caribe - Cartagena"? Put it as "Jamaica - Panamá - Colombia"?
```
...
Norteamérica <- c("Canadá", "Estados Unidos")
Centroamérica <- c("Costa Rica", "Cuba", "Guatemala", "Islas San Martin", "Jamaica", "Jamaica - Panamá - Isla Caimán", "Jamaica - Panamá - Islas del caribe - Cartagena", "México", "Panamá", "Panamá - Jamaica", "Puerto Rico", "República Dominicana", "Venezuela")
Sudamerica <- c("Argentina", "Aruba", "Brasil", "Chile", "Ecuador", "Perú")
