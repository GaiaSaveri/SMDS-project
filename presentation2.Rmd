...
pred.pois <- poisson1c$fitted.values
res.st <- (cases$`Cumulative cases`[1:120] - pred.pois)/sqrt(pred.pois)
#n=120, k=2, n-k=118
print(paste("Estimated overdispersion", sum(res.st^2)/118))
poisson1.pred <- predict(poisson1c, newdata = cases[120:126, ], type="response")
...
attach(data1)
conf.df_pois2 <- predict.confidence(poisson2, newdata = data1[1:120, ])
...
```{r pre_acc_pois3, warning=FALSE, include=FALSE}
attach(data1)
conf.df_pois3 <- predict.confidence(poisson3, newdata = data1[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
n <- 120
freq_coverage_pois3 <- sum(`Cumulative cases` >= conf.df_pois3[, 2] & `Cumulative cases` <= conf.df_pois3[, 3])
freq_coverage_pois3 <- freq_coverage_pois3/n
detach(data1)
```

```{r precc_acc_pois3_plot, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois3$lwr[1:122], ymax = conf.df_pois3$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
```{r pre_acc_pois4, warning=FALSE, include=FALSE}
attach(data1)
conf.df_pois4 <- predict.confidence(poisson4, newdata = data1[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
n <- 120
freq_coverage_pois4 <- sum(`Cumulative cases` >= conf.df_pois4[, 2] & `Cumulative cases` <= conf.df_pois4[, 3])
freq_coverage_pois4 <- freq_coverage_pois4/n
detach(data1)
```

```{r precc_acc_pois4_plot, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois4$lwr[1:122], ymax = conf.df_pois4$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
```{r pre_acc_pois4bis, warning=FALSE, include=FALSE}
attach(data1)
conf.df_pois4bis <- predict.confidence(poisson4bis, newdata = data1[1:120, ])
...
freq_coverage_pois4bis <- freq_coverage_pois4bis/n
detach(data1)
```
...
```{r pre_acc_pois5, warning=FALSE, include=FALSE}
attach(data1)
conf.df_pois5 <- predict.confidence(poisson5, newdata = data1[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
n <- 120
freq_coverage_pois5 <- sum(`Cumulative cases` >= conf.df_pois5[, 2] & `Cumulative cases` <= conf.df_pois5[, 3])
freq_coverage_pois5 <- freq_coverage_pois5/n
detach(data1)
```

```{r precc_acc_pois5_plot, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois5$lwr[1:122], ymax = conf.df_pois5$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
autocor <- data.frame(Poisson_1,Poisson_2,Poisson_3,Poisson_4)
library(reshape2)
x1 <- melt(autocor)
...
```{r pre_acc_qpois, warning=FALSE, include=FALSE}
attach(data1)
conf.df_qpois <- predict.confidence(poisson1quasi, newdata = data1[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
n <- 120
freq_coverage_qpois <- sum(`Cumulative cases` >= conf.df_qpois[, 2] & `Cumulative cases` <= conf.df_qpois[, 3])
freq_coverage_qpois <- freq_coverage_qpois/n
detach(data1)
```

```{r precc_acc_qpois_plot, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_qpois$lwr[1:122], ymax = conf.df_qpois$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
```{r pre_acc_qpois2, warning=FALSE, include=FALSE}
attach(data1)
conf.df_qpois2 <- predict.confidence(poisson2quasi, newdata = data1[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
n <- 120
freq_coverage_qpois2 <- sum(`Cumulative cases` >= conf.df_qpois2[, 2] & `Cumulative cases` <= conf.df_qpois2[, 3])
freq_coverage_qpois2 <- freq_coverage_qpois2/n
detach(data1)
```

```{r precc_acc_qpois2_plot, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_qpois2$lwr[1:122], ymax = conf.df_qpois2$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
```{r pre_acc_nb1, warning=FALSE, include=FALSE}
attach(data1)
conf.df_nb1 <- predict.confidence(nb1, newdata = data1[1:120, ])
...
freq_coverage_nb1 <- freq_coverage_nb1/n
detach(data1)
```
...
attach(data1)
conf.df_nb2 <- predict.confidence(nb2, newdata = data1[1:120, ])
...
```{r pre_acc_nb3, warning=FALSE, include=FALSE}
attach(data1)
conf.df_nb3 <- predict.confidence(nb3, newdata = data1[1:120, ])
#conf.df_nb <- predict.glm(nb5, newdata = as.data.frame(cbind(data1$`Elapsed time`, data1$`Cumulative cases`)), interval = 'confidence')
n <- 120
freq_coverage_nb3 <- sum(`Cumulative cases` >= conf.df_nb3[, 2] & `Cumulative cases` <= conf.df_nb3[, 3])
freq_coverage_nb3 <- freq_coverage_nb3/n
detach(data1)
```
...
```{r pre_acc_nb4, warning=FALSE, include=FALSE}
attach(data1)
...
freq_coverage_nb4 <- freq_coverage_nb4/n
detach(data1)
```
...
```{r pre_acc_nb5bis, warning=FALSE, include=FALSE}
attach(data1)
