...
attach(cases)
conf.df_pois1B <- add_pi(cases, poisson1B, names=c("lwr", "upr"))
n <- 126
freq_coverage_pois1B <- sum(`New cases/day` >= conf.df_pois1B[, 6] & `New cases/day` <= conf.df_pois1B[, 7])
...
ggplot(data1, aes(`Elapsed time`, `New cases/day`)) +
  ...
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois1B$lwr[120:126],
              ...
              data = data1[120:126, ],
              fill = color_scheme_get("red")[[2]]) +
  geom_line(aes(x = `Elapsed time`, y = conf.df_pois1B$pred[1:120]),
              data = data1[1:120, ],
              color = color_scheme_get("blue")[[4]], 
              size = 1.1) +
...
```{r pi_poisson1B, echo=FALSE, warning=FALSE}
test_cases<-cases[120:126,]
poisson1B.pi <- glm(`New cases/day` ~ `Elapsed time`, data=test_cases, family=poisson)
test_cases %>% 
  add_pi(poisson1B.pi, names=c("lpb", "upb")) %>%
...
attach(cases)
conf.df_pois1C <- add_pi(cases, poisson1C, names=c("lwr", "upr"))
n <- 126
freq_coverage_pois1C <- sum(`Cumulative cases` >= conf.df_pois1C[, 6] & `Cumulative cases` <= conf.df_pois1C[, 7])
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  ...
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois1C$lwr[120:126],
              ...
              data = data1[120:126, ],
              fill = color_scheme_get("red")[[2]]) +
  geom_line(aes(x = `Elapsed time`, y = conf.df_pois1C$pred[1:120]),
              data = data1[1:120, ],
              color = color_scheme_get("blue")[[4]], 
              size = 1.1) +
...
```{r pi_poisson1C, echo=FALSE, warning=FALSE}
test_cases<-cases[120:126,]
poisson1C.pi <- glm(`Cumulative cases` ~ `Elapsed time` + I(`Elapsed time`^2), data=test_cases, family=poisson)
test_cases %>% 
  add_pi(poisson1C.pi, names=c("lpb", "upb")) %>%
  ggplot(aes(x=test_cases$`Elapsed time`, y=test_cases$`Cumulative cases`)) +
  geom_point(size=2) +
  geom_point(aes(y=unname(poisson1C.pred)), size=2, color="maroon") +
  geom_ribbon(aes(ymin=lpb-(abs(lpb-unname(poisson1C.pred)))-abs(lpb-upb)/2,
                  ymax=upb-(abs(upb-unname(poisson1C.pred)))+abs(upb-lpb)/2),
...
```{r pois5day, results = "hold", echo=TRUE, warning=FALSE}
...
```{r summary_poisson_cumulative, echo=TRUE, warning=FALSE}
library(kableExtra)
poisson.cumulative.summary<-data.frame(Model="Cumulative cases ~ Elapsed time", AIC = 21915, RMSE = 1760)
poisson1C.summary<-c("Cumulative cases ~ Elapsed time + Elapsed time^2", 12062, 6940)
...
poisson.cumulative.summary<-rbind(poisson.cumulative.summary, poisson1C.summary)
...
```{r summary_poisson_cases, echo=TRUE, warning=FALSE}
...
Poisson_5b <- c()
for(i in 1:10000){ Poisson_5b <- c(Poisson_5b, acf5b[[c(i,1,2)]])}
...
autocor <- data.frame(Poisson_1,Poisson_1b,Poisson_2,Poisson_3,Poisson_4,Poisson_5,Poisson_5b,Poisson_6)
library(reshape2)
x1 <- melt(autocor)
...
```{r summary_quasi_poisson_cumulative, echo=TRUE, warning=FALSE}
...
```{r summary_quasi_poisson_cases, echo=TRUE, warning=FALSE}
...
```{r pi_nb5bis, echo=FALSE, warning=FALSE}
test_data1 %>% 
...
```{r summary_nb_cumulative, echo=TRUE, warning=FALSE}
...
```{r summary_nb_cases, echo=TRUE, warning=FALSE}
