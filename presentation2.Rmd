...
attach(data1)
conf.df_pois1 <- predict.confidence(poisson1, newdata = data1[1:120, ])
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois1$lwr[1:122], ymax = conf.df_pois1$upr[1:122]),
              data = data1[1:122, ],
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois1A$lwr[1:122], ymax = conf.df_pois1A$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
attach(data1)
conf.df_pois1B <- predict.confidence(poisson1B, newdata = data1[1:120, ])
...
#n=120, k=2, n-k=118
print(paste("Estimated overdispersion", sum(res.st1C^2)/118))
...
attach(data1)
conf.df_pois1C <- predict.confidence(poisson1C, newdata = data1[1:120, ])
#conf.df_pois <- predict.glm(poisson4, newdata = data1, interval = "confidence", type="response")
n <- 120
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois1C$lwr[1:122], ymax = conf.df_pois1C$upr[1:122]),
              data = data1[1:122, ],
              fill = color_scheme_get("blue")[[2]]) + 
