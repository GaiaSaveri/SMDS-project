...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  ...
  scale_x_discrete(limit = c(0, 21, 42, 63, 84, 105, 120, 126),
...
ggplot(data1, aes(`Elapsed time`, `Cumulative cases`)) +
  ...
  geom_point(aes(x = `Elapsed time`, y = `Cumulative cases`)) +
  expand_limits(x = 28) +
  ggtitle("Poisson regression", subtitle="Cumulative cases ~ Elapsed time + Elapsed time^2") + xlab("Days") + ylab("Cumulative cases") +
  scale_x_discrete(limit = c(0, 21, 42, 63, 84, 105, 120, 126),
                labels = c("06-3", "29-3", "19-4", "10-5", "31-5", "21-6", "06-7", "12-7")) +
  # facet_wrap('reg', scales ='free') +
  theme(strip.text.x = element_text(size = 12, colour = "black"),
...
```{r precc_acc_pois4bis_new_plot, echo=FALSE, warning=FALSE}
ggplot(data1, aes(`Elapsed time`, `New cases/day`)) +
...
autocor <- data.frame(Poisson_1,Poisson_1b,Poisson_2,Poisson_3,Poisson_4,Poisson_4b,Poisson_5)
library(reshape2)
x1 <- melt(autocor)
...
ggplot(data1, aes(`Elapsed time`, `New cases/day`)) +
  geom_ribbon(aes(x = `Elapsed time`, ymin = conf.df_pois1qbis$lwr[1:120], ymax = conf.df_pois1qbis$upr[1:120]),
              data = data1[1:120, ],
              fill = color_scheme_get("blue")[[2]]) + 
...
```{r NB_posterior_cases2, echo=TRUE, warning=FALSE, eval=FALSE}
