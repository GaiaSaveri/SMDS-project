...
So far we have estimated the cumulative number of cases, let's see what we can get by modifying the Stan code in order to predict the number of new cases each day (namely the quantity $S(t)-S(t+1) = All(t)-Confirmed(t)-All(t+1)+Confirmed(t+1) = Confirmed(t+1)-Confirmed(t)$). In this case we say that we are modeling *incidence data* (reference 1). 

```{r incidence_fit, echo=TRUE, warning=FALSE}
#compile the model 
...
pars<-c("thet[0]", "theta[1]") #, "R0", "recovery_time")
